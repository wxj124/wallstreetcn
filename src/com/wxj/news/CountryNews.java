...
public class CountryNews extends Thread{
	private static String urlRegex = "\"id\":\"(.*?)\".*?\"type\":\"(.*?)\".*?\"contentHtml\":\"<p>(.*?)<\\\\/p>\".*?\"categorySet\":\"(.*?)\".*?";
//	private static String urlRegex = ".*?\"type\":\"(.*?)\".*?\"contentHtml\":\"<p>(.*?)<\\\\/p>\".*?\"categorySet\":\"(.*?)\".*?";
	...
	public static List<Map> htmlFiter(String html) {
		...
		while (m.find()) {
			...
			map_save.put("cid", m.group(1));
			map_save.put("type", m.group(2));
			map_save.put("content", m.group(3));
			map_save.put("categoryset", m.group(4));
			list.add(map_save);
		}
		return list;
	}
